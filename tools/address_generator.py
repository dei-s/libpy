...
parser = argparse.ArgumentParser(description='Waves address generator')
parser.add_argument('-c', '--count', type=int, help='number of addresses to generate')
parser.add_argument('-t', '--testnet', action='store_true', help='generate testnet addresses')

args = parser.parse_args()
n_addresses = args.count
if not n_addresses:
	n_addresses = 1
chainId = "T" if args.testnet else "W"
...
def generatePhrase():
	...
	for i in range(5):
	  ...
	  w3 = (((((x / wordCount) >> 0) / wordCount) >> 0) + w2) % wordCount
